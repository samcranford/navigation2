...
#include "nav2_navfn_extension_planner/navfn_extension_planner.hpp"
...
#include "nav2_navfn_extension_planner/navfn_extension.hpp"
...
namespace nav2_navfn_extension_planner
{

NavfnExtensionPlanner::NavfnExtensionPlanner()
...
{
  RCLCPP_INFO(
    node_->get_logger(), "Destroying plugin %s of type NavfnExtensionPlanner",
    name_.c_str());
}

void
NavfnExtensionPlanner::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(
    node_->get_logger(), "Configuring plugin %s of type NavfnExtensionPlanner",
    name_.c_str());
  ...
  declare_parameter_if_not_declared(node_, name + ".low_cost_ignore", rclcpp::ParameterValue(0));
  node_->get_parameter(name + ".low_cost_ignore", low_cost_ignore_);

  // Create a planner based on the new costmap size
  planner_ = std::make_unique<NavFnExtension>(
  ...
  parameter_event_sub_ = parameters_client_->on_parameter_event(
    std::bind(&NavfnExtensionPlanner::on_parameter_event_callback, this, _1));
}

void
NavfnExtensionPlanner::activate()
{
  RCLCPP_INFO(
    node_->get_logger(), "Activating plugin %s of type NavfnExtensionPlanner",
    name_.c_str());
}

void
NavfnExtensionPlanner::deactivate()
{
  RCLCPP_INFO(
    node_->get_logger(), "Deactivating plugin %s of type NavfnExtensionPlanner",
    name_.c_str());
}

void
NavfnExtensionPlanner::cleanup()
{
  RCLCPP_INFO(
    node_->get_logger(), "Cleaning up plugin %s of type NavfnExtensionPlanner",
    name_.c_str());
  planner_.reset();
}

nav_msgs::msg::Path NavfnExtensionPlanner::createPlan(
...
bool
NavfnExtensionPlanner::isPlannerOutOfDate()
{
  if (!planner_.get() ||
...
bool
NavfnExtensionPlanner::makePlan(
  ...
  nav_msgs::msg::Path & plan)
{
  ...
  int goal_cost = costmap_->getCost(mx,my);

  // make sure to resize the underlying array that NavfnExtension uses
  planner_->setNavArr(
    costmap_->getSizeInCellsX(),
    costmap_->getSizeInCellsY());

  planner_->setCostmap(costmap_->getCharMap(), true, allow_unknown_, goal_cost, low_cost_ignore_);
...
void
NavfnExtensionPlanner::smoothApproachToGoal(
...
bool
NavfnExtensionPlanner::getPlanFromPotential(
  const geometry_msgs::msg::Pose & goal,
  nav_msgs::msg::Path & plan)
{
  // clear the plan, just in case
  ...
  if (!worldToMap(wx, wy, mx, my)) {
    RCLCPP_WARN(
      node_->get_logger(),
      "The goal sent to the navfn extension planner is off the global costmap."
      " Planning will always fail to this goal.");
    return false;
  }
...
double
NavfnExtensionPlanner::getPointPotential(const geometry_msgs::msg::Point & world_point)
{
  unsigned int mx, my;
...
// bool
// NavfnExtensionPlanner::validPointPotential(const geometry_msgs::msg::Point & world_point)
// {
//   return validPointPotential(world_point, tolerance_);
// }

// bool
// NavfnExtensionPlanner::validPointPotential(
...
bool
NavfnExtensionPlanner::worldToMap(double wx, double wy, unsigned int & mx, unsigned int & my)
{
...
void
NavfnExtensionPlanner::mapToWorld(double mx, double my, double & wx, double & wy)
{
...
void
NavfnExtensionPlanner::clearRobotCell(unsigned int mx, unsigned int my)
{
...
void
NavfnExtensionPlanner::on_parameter_event_callback(
...
PLUGINLIB_EXPORT_CLASS(nav2_navfn_extension_planner::NavfnExtensionPlanner, nav2_core::GlobalPlanner)
