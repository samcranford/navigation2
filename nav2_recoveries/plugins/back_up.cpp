...
BackUp::BackUp() : Recovery<BackUpAction>(), feedback_(std::make_shared<BackUpAction::Feedback>()) {
}
...
Status BackUp::onRun(const std::shared_ptr<const BackUpAction::Goal> command) {
    if (command->target.y != 0.0 || command->target.z != 0.0) {
        RCLCPP_INFO(node_->get_logger(),
                    "Backing up in Y and Z not supported, "
                    "will only move in X.");
    }
...
Status BackUp::onCycleUpdate() {
    ...
    if (time_remaining.seconds() < 0.0 && command_time_allowance_.seconds() > 0.0) {
        stopRobot();
        RCLCPP_WARN(node_->get_logger(), "Exceeded time allowance before reaching the BackUp goal - Exiting BackUp");
        return Status::FAILED;
    }
...
bool BackUp::isCollisionFree(const double& distance, geometry_msgs::msg::Twist* cmd_vel, geometry_msgs::msg::Pose2D& pose2d) {
