...
Spin::Spin() : Recovery<SpinAction>(), feedback_(std::make_shared<SpinAction::Feedback>()), prev_yaw_(0.0) {
}
...
Status Spin::onCycleUpdate() {
    ...
    if (time_remaining.seconds() < 0.0 && command_time_allowance_.seconds() > 0.0) {
        stopRobot();
        RCLCPP_WARN(node_->get_logger(), "Exceeded time allowance before reaching the Spin goal - Exiting Spin");
        return Status::FAILED;
    }
...
bool Spin::isCollisionFree(const double& relative_yaw, geometry_msgs::msg::Twist* cmd_vel, geometry_msgs::msg::Pose2D& pose2d) {
